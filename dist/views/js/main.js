...
var resizePizzas = function(size) {
  ...
  // Created global variables to select the randomPizzaContainer class from the DOM
  // and used the selector getElementsByClassName for optimization 
  var pizzaSizeGroup = document.getElementsByClassName('randomPizzaContainer');
  var pizzaGroupLength = pizzaSizeGroup.length;

  function changePizzaSizes(size) {

    var newPizzaWidth; // new variable to hold the new width value for the pizzas

    switch(size){ // define new sizes for the slider and return the newPizzaWidth value
      case '1':
        newPizzaWidth = 25;
        break;
    ...
    // loop over the pizzaSizeGroup and assign the 
    // new width from the newPizzaWidth value
    // returned from the switch condition
    for (var i = 0; i < pizzaGroupLength; i++) {
      pizzaSizeGroup[i].style.width = newPizzaWidth + '%';
    }
  }
...
// Setting up the code structure for using requestAnimationFrame
// to help with optimizing the paint section
window.animating = false;

function pizzasAnimating() {
  if(!window.animating) {

    // if requestAnimationFrame is true 
    // ask for the browser to call the updatePositions function 
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover'); // Vanilla JS DOM querying function - more efficient for optimizing

  // Caching (moving items.length and document.body.scrollTop outside of the loop)
  ...
  var scrollArray = []; //store values after loop in array

  // Loops through five values that were
  // previously checked with the module (i % 5) and console.log on the dev tools
  for (var i = 0; i < 5; i++) { 
    
    scrollArray.push(Math.sin((top / 1250) + i)); // loops and push all the five values to the empty scrollArray
  }

  // loop over length array of pizzas and return result
  for (var i = 0; i < len; i ++){
    var phase = scrollArray[i % 5];
    // CSS3 Hack for website optimization (transform = translateX());
  ...
  // sets the global object to an initial value of false 
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Declaring variables outside the for loop for web optimization
  var pizzasToAppend = document.getElementById('movingPizzas1');
  ...
  for (var i = 0; i < backgroundPizzas; i++) {
    ...
    pizzasToAppend.appendChild(elem);
